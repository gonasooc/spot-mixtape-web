...
export const site = {
  ...
  /**
   * 현재 Supabase 요금제의 백업·로그 보관 기간. 요금제를 바꾸면 반드시 함께
   * 고친다. 실제 설정과 다르면 허위 고지가 된다.
   */
  backupRetention:
    "백엔드는 Supabase 무료 플랜으로 운영되며 자동 백업을 제공하지 않습니다. 따라서 별도의 백업 사본이 보관되지 않습니다. 서비스 운영 로그는 1일간 보관된 뒤 삭제됩니다.",
  /**
   * 삭제 후 잔존 데이터. 자동 백업이 없는 무료 플랜이므로 백업 사본은 남지
   * 않는다. 법령상 보존 의무가 있는지는 법률 검토 대상이다.
   */
  deletionRetention:
    "자동 백업을 사용하지 않으므로 삭제된 콘텐츠의 백업 사본은 남지 않으며, 앱 콘텐츠를 의도적으로 보관하지 않습니다. 삭제 처리 과정에서 생성된 운영 로그는 위 로그 보관 기간이 지나면 삭제되고, 이메일로 접수한 삭제 요청 기록은 처리 확인에 필요한 범위에서만 보관합니다.",
  /** 계정 삭제 요청 처리 목표 기한 (일) */
  deletionSlaDays: 30,
